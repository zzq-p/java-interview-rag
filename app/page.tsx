...
import { ArrowLeft, ArrowRight, BookOpen, BrainCircuit, CheckCircle2, ChevronRight, Clock3, FileCode2, FileText, Library, Loader2, Plus, RotateCcw, Send, Sparkles, UploadCloud } from "lucide-react";
...
type Question = { id: string; topic: string; question: string; evaluationPoints: string[]; referenceAnswer: string; excerpt: string; documentName: string; position: number; difficulty: string };
...
export default function Home() {
  ...
  const loadDocuments = useCallback(async () => {
    ...
    } catch { /* The built-in sample remains usable while storage initializes. */ }
  }, []);
  ...
  async function evaluateAnswer() {
    ...
    setEvaluating(true);
    try {
      const response = await fetch("/api/evaluate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question: current.question, answer, excerpt: current.excerpt, evaluationPoints: current.evaluationPoints }) });
  ...
  function moveQuestion(direction: 1 | -1) {
    ...
    setQuestionIndex(next); setAnswer(""); setFeedback(null);
  }
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <header className="flex h-16 items-center justify-between border-b border-border px-5 lg:px-8">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground"><span className="hidden sm:inline">{documents.length ? "资料库已就绪" : "等待上传资料"}</span><span className={`size-2 rounded-full ${documents.length ? "bg-[#b8f34a] shadow-[0_0_0_4px_rgba(184,243,74,.15)]" : "bg-[#b7b4aa]"}`} /></div>
      </header>

      <div className="grid min-h-[calc(100vh-4rem)] lg:grid-cols-[292px_minmax(0,1fr)]">
        <aside className="border-b border-border bg-[#f4f1e9] px-5 py-5 lg:border-b-0 lg:border-r lg:px-6 lg:py-7">
          <div className="mb-5 flex items-center justify-between"><div><p className="section-label">知识资料</p><p className="mt-1 text-sm text-muted-foreground">{documents.length ? `${documents.length} 份文档 · ${totalChunks} 个片段` : "上传资料后按内容出题"}</p></div>
            <Dialog open={uploadOpen} onOpenChange={setUploadOpen}><DialogTrigger asChild><Button size="icon-sm" aria-label="上传文档" className="rounded-full"><Plus /></Button></DialogTrigger><DialogContent className="border-0 bg-[#fbfaf5] sm:max-w-md"><DialogHeader><DialogTitle className="font-display text-2xl">添加面试资料</DialogTitle><DialogDescription>上传 Java 笔记或团队文档，系统会保存原文件、切分文本并用于检索出题。</DialogDescription></DialogHeader><label className={`mt-2 flex min-h-44 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#97a09a] bg-white p-6 text-center transition hover:border-primary hover:bg-[#f8faf3] ${uploading ? "pointer-events-none opacity-60" : ""}`}>{uploading ? <Loader2 className="mb-3 size-7 animate-spin text-primary" /> : <UploadCloud className="mb-3 size-7 text-primary" />}<span className="font-medium">{uploading ? "正在切分并建立索引…" : "拖入文件，或点击选择"}</span><span className="mt-1 text-xs text-muted-foreground">TXT、Markdown、Java、JSON、XML、CSV，单个不超过 2MB</span><input className="sr-only" type="file" disabled={uploading} accept=".txt,.md,.java,.json,.xml,.csv,.properties" onChange={(event) => void handleUpload(event.target.files?.[0])} /></label></DialogContent></Dialog>
          </div>

          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
            {documents.map((item) => <DocumentCard key={item.id} item={item} />)}
            {!documents.length && <div className="rounded-2xl border border-dashed border-[#c8c5ba] bg-white/60 px-4 py-5 text-center"><FileText className="mx-auto size-5 text-muted-foreground" /><p className="mt-2 text-sm font-semibold">还没有资料</p><p className="mt-1 text-xs leading-5 text-muted-foreground">上传你的 Java 笔记或团队文档后，系统才会检索并调用模型出题。</p><Button variant="outline" size="sm" className="mt-3 rounded-lg bg-white" onClick={() => setUploadOpen(true)}>上传第一份资料</Button></div>}
          </div>

          <div className="mt-6 rounded-2xl bg-[#142422] p-4 text-white"><div className="flex items-center gap-2 text-sm font-semibold"><Library className="size-4 text-[#b8f34a]" />检索状态</div><p className="mt-3 text-xs leading-5 text-white/60">{documents.length ? "文档已完成文本切片。每次出题都会检索相关片段、调用模型并保留出处。" : "资料库为空。不会使用任何预置内容，也不会在没有检索依据时出题。"}</p><div className="mt-4 flex items-center justify-between text-xs"><span className="text-white/60">索引覆盖</span><span>{documents.length ? "100%" : "0%"}</span></div><Progress value={documents.length ? 100 : 0} className="mt-2 h-1 bg-white/10 [&_[data-slot=progress-indicator]]:bg-[#b8f34a]" /></div>
        </aside>

        <section className="min-w-0 px-5 py-7 sm:px-8 lg:px-12 lg:py-9">
          <div className="mx-auto max-w-5xl">
            <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><p className="section-label flex items-center gap-2"><Sparkles className="size-3.5" />基于资料智能出题</p><h1 className="font-display mt-2 max-w-2xl text-3xl font-bold tracking-[-.045em] sm:text-4xl">把读过的 Java 知识，<br className="hidden sm:block" />练成能说出口的答案。</h1></div>
              <div className="grid grid-cols-[1fr_auto_auto] gap-2"><Input aria-label="出题主题" value={topic} onChange={(event) => setTopic(event.target.value)} placeholder="主题，如 JVM" className="h-10 min-w-0 rounded-xl bg-white sm:w-36" /><Select value={difficulty} onValueChange={setDifficulty}><SelectTrigger aria-label="难度" className="h-10 w-24 rounded-xl bg-white"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="入门">入门</SelectItem><SelectItem value="中等">中等</SelectItem><SelectItem value="进阶">进阶</SelectItem></SelectContent></Select><Button disabled={generating || !documents.length} onClick={() => void generateQuestions()} className="h-10 rounded-xl bg-[#142422] px-4 text-white hover:bg-[#213936]">{generating ? <Loader2 className="animate-spin" /> : <ArrowRight />}<span className="hidden sm:inline">调用模型出题</span></Button></div>
            </div>
            ...
            <div className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_280px]">
              {current ? <article className="overflow-hidden rounded-[22px] border border-[#d8d7cf] bg-white shadow-[0_20px_50px_rgba(26,42,40,.07)]">
                ...
                <div className="p-5 sm:p-7"><h2 className="font-display text-xl font-bold leading-8 tracking-[-.02em] sm:text-2xl">{current.question}</h2><div className="mt-5 rounded-xl border-l-4 border-[#b8f34a] bg-[#f3f6ee] p-4"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.08em] text-[#58703b]"><BookOpen className="size-3.5" />检索依据</div><p className="mt-2 line-clamp-3 text-sm leading-6 text-[#46514d]">“{current.excerpt}”</p><p className="mt-2 text-xs text-muted-foreground">{current.documentName} · 片段 {current.position}</p></div><Textarea value={answer} onChange={(event) => setAnswer(event.target.value)} disabled={Boolean(feedback) || evaluating} placeholder="用面试表达方式作答：先下结论，再解释原理，最后补充使用场景或边界条件……" className="mt-5 min-h-36 resize-y rounded-xl bg-[#fbfaf7] p-4 text-base leading-7" />{feedback && <FeedbackPanel feedback={feedback} />}<div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div className="flex gap-1"><Button variant="ghost" size="sm" onClick={() => moveQuestion(-1)} disabled={questions.length < 2}><ArrowLeft />上一题</Button><Button variant="ghost" size="sm" onClick={() => moveQuestion(1)} disabled={questions.length < 2}>下一题<ChevronRight /></Button></div>{feedback ? <Button variant="outline" className="rounded-xl" onClick={() => { setAnswer(""); setFeedback(null); }}><RotateCcw />重新作答</Button> : <Button disabled={evaluating} className="rounded-xl bg-[#b8f34a] text-[#142422] hover:bg-[#a8e23d]" onClick={() => void evaluateAnswer()}>{evaluating ? <Loader2 className="animate-spin" /> : <Send />}调用模型点评</Button>}</div></div>
              </article> : <div className="grid min-h-[470px] place-items-center rounded-[22px] border border-dashed border-[#c8c5ba] bg-white/70 p-7 text-center shadow-[0_20px_50px_rgba(26,42,40,.04)]"><div className="max-w-sm"><div className="mx-auto grid size-14 place-items-center rounded-2xl bg-[#e8f9c9] text-[#315027]"><UploadCloud className="size-6" /></div><h2 className="font-display mt-5 text-2xl font-bold">先上传你自己的资料</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">系统不会使用预置题库。上传后会从你的文档中检索相关片段，再调用模型生成题目和答案点评。</p><Button className="mt-5 rounded-xl" onClick={() => setUploadOpen(true)}><Plus />上传资料</Button></div></div>}
