...
function splitIntoChunks(source: string) {
  ...
  return result.flatMap((part) => part.length <= 1100 ? [part] : part.match(/[\s\S]{1,900}/g) ?? []).slice(0, 120);
}

export async function GET() {
  try {
    const rows = await getDb().select().from(documents).orderBy(desc(documents.createdAt), desc(documents.id)).limit(30);
...
export async function POST(request: Request) {
  try {
    ...
    await db.insert(chunks).values(pieces.map((chunkContent, position) => ({ documentId: document.id, position, content: chunkContent })));
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!Number.isInteger(id)) return Response.json({ error: "文档编号无效。" }, { status: 400 });
    const db = getDb();
