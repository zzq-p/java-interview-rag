...
export async function retrieveChunks(query: string, limit = 8) {
  ...
  const selectChunks = () => getDb().select({ id: chunks.id, content: chunks.content, documentName: documents.name, position: chunks.position }).from(chunks).innerJoin(documents, eq(chunks.documentId, documents.id));
  const matching = queryTerms.length
    ? await selectChunks().where(or(...queryTerms.flatMap((term) => [like(chunks.content, `%${term}%`), like(documents.name, `%${term}%`)]))).orderBy(desc(chunks.id)).limit(200)
    : [];
  // A topic with no exact matches should still allow practice from the latest uploaded material.
  const rows = matching.length ? matching : await selectChunks().orderBy(desc(chunks.id)).limit(Math.max(limit, 50));
